...
def setup_clients():
    ...
    if GEMINI_API_KEY:
        try:
            ...
            # แก้ไขตรงนี้: ใช้ชื่อมาตรฐาน gemini-1.5-flash เท่านั้น
            clients['gemini'] = genai.GenerativeModel('gemini-1.5-flash')
        except: clients['gemini'] = None
...
async def get_ai_res(name, prompt):
    try:
        if name == 'gemini' and ai_clients.get('gemini'):
            # ใช้ asyncio.to_thread เพื่อความเสถียร
        ...
        return "❌ ไม่ได้ตั้งค่า API"
    except Exception as e:
        return f"❌ Error: {str(e)}"

async def handle_request(update: Update, cmd: str, prompt: str):
    if not prompt:
        await update.message.reply_text(f"ใช้คำสั่ง: {cmd} ตามด้วยคำถาม")
        return
