...
async def ask_gemini(prompt):
    model = genai.GenerativeModel('gemini-1.5-flash-latest') # แก้ชื่อโมเดลแล้ว
...
async def ask_claude(prompt):
    ...
    return await loop.run_in_executor(None, sync_claude) # แก้จาก to_thread
...
async def ask_gpt(prompt):
    ...
    return await loop.run_in_executor(None, sync_gpt) # แก้จาก to_thread
