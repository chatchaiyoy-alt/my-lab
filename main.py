...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("สวัสดีครับท่าน บอท 3 สมอง แก้สภาล่มแล้ว พร้อมลุย")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if text.startswith("claude "):
            ...
            prompt = text[4:]
            await update.message.reply_text("รับทราบ... 3 สมองกำลังประมวลผล สภาไม่ล่มแล้ว")
