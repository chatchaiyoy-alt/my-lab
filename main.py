...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = update.message.text.lower().strip() # แปลงเป็นตัวเล็ก + ตัดช่องว่าง
    try:
        if text.startswith("claude"):
            prompt = update.message.text[6:].strip() # ใช้ text เดิม ไม่ใช่ text ที่ lower แล้ว
            ...
            prompt = update.message.text[6:].strip()
            if not prompt: prompt = "สวัสดี"
